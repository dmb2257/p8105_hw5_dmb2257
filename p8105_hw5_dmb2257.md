...
```
    ...
    ## ℹ In argument: `output_lists = map(.x = mu, ~rerun(5000, sim_mu_hat(.x)))`.
    ## Caused by warning:
    ## ! `rerun()` was deprecated in purrr 1.0.0.
    ## ℹ Please use `map()` instead.
    ##   # Previously
    ##   rerun(5000, sim_mu_hat(.x))
    ## 
    ##   # Now
    ##   map(1:5000, ~ sim_mu_hat(.x))
...
increases. The change in the power of the test is steep at the lower
true mu values and becomes more gradual as the true value of mu
increases. At mu= 4, the power of the test begins to plateau towards y =
1.
